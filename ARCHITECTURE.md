...
| 21 | Reference-flow fidelity & the local-git boundary | Built-ins mirror the **Beetegre-V2 reference flow**: `task-new` CREATES, `task-start` starts an **existing** item on its **core-derived canonical branch** (`<prefix>/<id>-<slug>`, from the type role — never invented, unset for containers), `task-finish` opens the PR **without moving the role** (review state changes on MERGE). **Boundary:** Baron owns provider truth (work items, remote branches, PRs); the LOCAL working tree (status/fetch/switch/push) belongs to the harness/skill layer around the recipe call |
